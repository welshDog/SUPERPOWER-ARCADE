...
- **No-WebGL fallback:** ✅ FIXED 2026-07-17 (same day, follow-up commit). Previously, if `THREE.WebGLRenderer` construction threw (blocked GPU, very old device), `mount()` died mid-function *after* the UI was appended — the screen looked fine but slot clicks hit the `rings` TDZ and threw, soft-locking the combo at ⟁⟁⟁⟁; even a correct UNLOCK died at `isOpening` before `ctx.complete()`. Now `rings`/`isOpening`/`cameraShake` are initialized before the UI handlers close over them, and the entire THREE block (scene → animate → ResizeObserver) is wrapped in try/catch that degrades to UI-only with a console.warn. Locked by the "no-WebGL soft-lock regression" structural test in `tests/e2e.test.js` (suite: 89 green). Browser-verified with WebGL contexts stubbed to null: THREE errors, the catch logs "3D backdrop disabled", slots cycle, UNLOCK attempts run, the abandon path completes the chamber through to the reveal screen — and the normal WebGL path still renders the 3D vault.
